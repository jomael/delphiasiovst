...
;General
  ...
  ;Name and file
  Name "DJ Filter Installer"
  OutFile "DJ_Filter_Install.exe"
...
Section "VST-Plugin" SecVstPlugin
  ...
  ${If} ${RunningX64}
  File "..\Bin\Win64\VST\DJ Filter.dll"
  ${Else}
  File "..\Bin\Win32\VST\DJ Filter.dll"
  ...
  StrCpy $0 "$INSTDIR\DJ Filter.dll"
...
SkipDLLCall:
  ...
  ;Create uninstaller
  WriteUninstaller "$INSTDIR\Uninstall_DJ_Filter.exe"
SectionEnd
...
;Descriptions
  ...
  LangString TEXT_IO_SUBTITLE ${LANG_ENGLISH} "DJ Filter VST Plugin"

  LangString DESC_SecVstPlugin ${LANG_ENGLISH} "DJ Filter VST Plugin"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\DJ Filter.dll"
  Delete "$INSTDIR\DJ Filter.pdf"
