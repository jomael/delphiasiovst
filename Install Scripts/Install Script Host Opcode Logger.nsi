...
;General
  ...
  ;Name and file
  Name "Host Opcode Logger Installer"
  OutFile "Host_Opcode_Logger_Install.exe"
...
;Interface Settings
  ...
  !define PRODUCT_NAME "Host Opcode Logger"
...
Section "VST-Plugin" SecVstPlugin
  ...
  ${If} ${RunningX64}
  File "..\Bin\Win64\VST\Host Opcode Logger.dll"
  ${Else}
  File "..\Bin\Win32\VST\Host Opcode Logger.dll"
  ...
  StrCpy $0 "$INSTDIR\Host Opcode Logger.dll"
...
SkipDLLCall:
  ...
  ;Create uninstaller
  WriteUninstaller "$INSTDIR\Uninstall_Host_Opcode_Logger.exe"
SectionEnd
...
;Descriptions
  ...
  LangString TEXT_IO_SUBTITLE ${LANG_ENGLISH} "Host Opcode Logger VST Plugin"

  LangString DESC_SecVstPlugin ${LANG_ENGLISH} "Host Opcode Logger VST Plugin"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\Host Opcode Logger.dll"
  Delete "$INSTDIR\Host Opcode Logger.pdf"
