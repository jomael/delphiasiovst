...
uses Classes, RTLConsts, VocoderVoice;
...
type
  ...
  protected
    ...
    procedure Put(Index: Integer; VocoderVoice: TVocoderVoice);
    procedure Notify(VocoderVoice: TVocoderVoice; Action: TListNotification); virtual;
  ...
  public
    ...
    function Add(VocoderVoice: TVocoderVoice): Integer;
    ...
    function Extract(VocoderVoice: TVocoderVoice): TVocoderVoice;
    function First: TVocoderVoice;
    function IndexOf(VocoderVoice: TVocoderVoice): Integer;
    procedure Insert(Index: Integer; VocoderVoice: TVocoderVoice);
    function Last: TVocoderVoice;
    procedure Move(CurIndex, NewIndex: Integer);
    function Remove(VocoderVoice: TVocoderVoice): Integer;
    procedure Pack;
    procedure Sort(Compare: TListSortCompare);
    procedure Assign(ListA: TVoiceList; AOperator: TListAssignOp = laCopy; ListB: TVoiceList = nil);
    property Capacity: Integer read FCapacity write SetCapacity;
    property Count: Integer read FCount write SetCount;
    property Items[Index: Integer]: TVocoderVoice read Get write Put; default;
...
function TVoiceList.Add(VocoderVoice: TVocoderVoice): Integer;
begin
 Result := FCount;
 if Result = FCapacity then Grow;
 FList^[Result] := VocoderVoice;
 Inc(FCount);
 if VocoderVoice <> nil then Notify(VocoderVoice, lnAdded);
end;
...
function TVoiceList.First: TVocoderVoice;
begin
  Result := Get(0);
end;

function TVoiceList.Get(Index: Integer): TVocoderVoice;
begin
  if (Index < 0) or (Index >= FCount) then
    Error(@SListIndexError, Index);
  Result := FList^[Index];
...
function TVoiceList.IndexOf(VocoderVoice: TVocoderVoice): Integer;
begin
  Result := 0;
  while (Result < FCount) and (FList^[Result] <> VocoderVoice) do
    Inc(Result);
  if Result = FCount then
    Result := -1;
end;

procedure TVoiceList.Insert(Index: Integer; VocoderVoice: TVocoderVoice);
begin
 ...
 FList^[Index] := VocoderVoice;
 Inc(FCount);
 if VocoderVoice <> nil
  then Notify(VocoderVoice, lnAdded);
end;

function TVoiceList.Last: TVocoderVoice;
begin
  Result := Get(FCount - 1);
end;
...
procedure TVoiceList.Put(Index: Integer; VocoderVoice: TVocoderVoice);
var
  Temp: Pointer;
begin
 if (Index < 0) or (Index >= FCount)
  then Error(@SListIndexError, Index);
 if VocoderVoice <> FList^[Index] then
  begin
   Temp := FList^[Index];
   FList^[Index] := VocoderVoice;
...
function TVoiceList.Remove(VocoderVoice: TVocoderVoice): Integer;
begin
 Result := IndexOf(VocoderVoice);
...
function TVoiceList.Extract(VocoderVoice: TVocoderVoice): TVocoderVoice;
var I: Integer;
begin
 Result := nil;
 I := IndexOf(VocoderVoice);
 if I >= 0 then
  begin
   Result := VocoderVoice;
   FList^[I] := nil;
...
procedure TVoiceList.Notify(VocoderVoice: TVocoderVoice; Action: TListNotification);
begin
 if OwnsObjects then if Action = lnDeleted then VocoderVoice.Free;
end;
