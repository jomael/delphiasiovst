...
uses
  ...
  Forms, SyncObjs, Dialogs, DAV_Types, DAV_Complex, DAV_DspWindowFunctions,
  DAV_DspAnalogueFilterPrototypes, DAV_DspConvolution, DAV_DspFftReal2Complex,
...
begin
 PreDefined := AnsiString(
   GWindowFunctions[Round(Parameter[Index])].GetWindowFunctionName);
end;
...
begin
 ...
 for WindowIndex := 0 to Length(GWindowFunctions) - 1 do
  if Text = AnsiString(GWindowFunctions[Round(Parameter[Index])].GetWindowFunctionName) then
   begin
    Value := WindowIndex;
    Exit;
   end;
...
procedure TWeightingFiltersDataModule.ParameterTypeDisplay(
  Sender: TObject; const Index: Integer; var PreDefined: AnsiString);
const
  CTypeNames : array [0..3] of AnsiChar = ('A', 'B', 'C', 'D');
begin
...
var
  ...
  Freq  : Double;
  Scale : Double;
  Cmplx : array [0..1] of TComplexSingle;
begin
 ...
 try
  ...
  Cmplx[0] := ComplexPolar32(1, ArcTan2(-FFilterFreq[h].Im, FFilterFreq[h].Re) / h);
  Cmplx[1].Re := 1;
  Cmplx[1].Im := 0;
  ...
  for i := 1 to h - 1 do
   begin
    ...
    ComplexMultiplyInplace32(FFilterFreq[i], Cmplx[1]);
    ComplexMultiplyInplace32(Cmplx[1], Cmplx[0]);
   end;

  ComplexMultiplyInplace32(FFilterFreq[h], Cmplx[1]);
