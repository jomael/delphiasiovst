...
uses
  Math, DAV_VSTParameters, LinearPhaseLinkwitzRileyGUI;
...
begin
 ...
 FLinkwitzRileyFilter := TButterworthLowPassFilter.Create(2);
  ...
  try
   if FOversampled then
    begin
     ...
     InitialDelay := FConvolution.Latency + KernelSize div 2;
    end
   else
    begin
     FConvolution.LoadImpulseResponse(FilterKernel, 2 * KernelSize);
