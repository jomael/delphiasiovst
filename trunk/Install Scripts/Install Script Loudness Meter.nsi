...
;General
  ...
  ;Name and file
  Name "Loudness Meter Installer"
  OutFile "Loudness_Meter_Install.exe"
...
;Interface Settings
  ...
  !define PRODUCT_NAME "Loudness Meter"
...
Section "VST-Plugin" SecVstPlugin
  ...
  ${If} ${RunningX64}
  File "..\Bin\Win64\VST\Loudness Meter.dll"
  ${Else}
  File "..\Bin\Win32\VST\Loudness Meter.dll"
  ...
  StrCpy $0 "$INSTDIR\Loudness Meter.dll"
...
SkipDLLCall:
  ...
  ;Create uninstaller
  WriteUninstaller "$INSTDIR\Uninstall_Loudness_Meter.exe"
SectionEnd
...
;Descriptions
  ...
  LangString TEXT_IO_SUBTITLE ${LANG_ENGLISH} "Loudness Meter VST Plugin"

  LangString DESC_SecVstPlugin ${LANG_ENGLISH} "Loudness Meter VST Plugin"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\Loudness Meter.dll"
  Delete "$INSTDIR\Loudness Meter.pdf"
