...
Section "VST-Plugin" SecVstPlugin
  ...
  ${If} ${RunningX64}
  File "..\Bin\Win64\VST\HRTF 3D.dll"
  ${Else}
  File "..\Bin\Win32\VST\HRTF 3D.dll"
...
SkipDLLCall:
  ...
  ;Create uninstaller
  WriteUninstaller "$INSTDIR\Uninstall_HRTF_3D.exe"
SectionEnd
...
;Descriptions
  ...
  LangString TEXT_IO_SUBTITLE ${LANG_ENGLISH} "HRTF 3D VST Plugin"

  LangString DESC_SecVstPlugin ${LANG_ENGLISH} "HRTF 3D VST Plugin"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\HRTF 3D.dll"
  Delete "$INSTDIR\HRTF 3D.pdf"
