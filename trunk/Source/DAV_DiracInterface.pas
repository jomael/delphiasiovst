...
type
  ...
  protected
    ...
    function ReadFromChannelsCallback(Data: PDAVSingleFixedArray; const FrameCount: Integer): Integer;
  public
    ...
    property FormantFactor: Extended read GetFormantFactor write SetFormantFactor;
    property CompactSupport: Boolean read GetCompactSupport write SetCompactSupport;
    property CacheGranularity: Integer read GetCacheGranularity write SetCacheGranularity;
    property DoPitchCorrection: Boolean read GetPitchCorrection write SetPitchCorrection;
    property PitchCorrectionBasicTuning: Extended read GetPitchCorrectionBasicTuning write SetPitchCorrectionBasicTuning;
    property PitchCorrectionTolerance: Extended read GetPitchCorrectionTolerance write SetPitchCorrectionTolerance;
    property PitchCorrectionSlur: Extended read GetPitchCorrectionSlur write SetPitchCorrectionSlur;
    property Version: string read GetDiracVersion;
  end;

  // Prototypes
  PDiracreadFromChannelsCallback = ^TDiracreadFromChannelsCallback;
  TDiracreadFromChannelsCallback = function(Data: PDAVSingleFixedArray; FrameCount: Integer; UserData: Pointer): Integer; cdecl;

  TDiracVersion = function: PChar; cdecl;
  TDiracCreate = function(Lambda, Quality, ChannelCount: Integer; SampleRate: Single; ReadFromChannelsCallback: PDiracreadFromChannelsCallback): PDiracHandle; cdecl;
  TDiracSetProperty = function(Selector: Integer; Value: Double; DiracHandle: PDiracHandle): Integer; cdecl;
  TDiracGetProperty = function(Selector: Integer; DiracHandle: PDiracHandle): Double; cdecl;
  TDiracReset = procedure(Clear: Boolean; DiracHandle: PDiracHandle); cdecl;
  TDiracProcess = function(AudioOut: PDAVSingleFixedArray; FrameCount: Integer; UserData: Pointer; DiracHandle: PDiracHandle): Integer; cdecl;
  TDiracDestroy = procedure(DiracHandle: PDiracHandle); cdecl;
  TDiracGetInputBufferSizeInFrames = function(DiracHandle: PDiracHandle): Integer; cdecl;
...
function ReadFromChannelsStaticCallback(Data: PDAVSingleFixedArray; FrameCount: Integer; UserData: Pointer): Integer; cdecl;
begin
...
begin
 ...
 FDiracHandle := DiracCreate(Integer(FLambda), Integer(FQuality), 1, FSampleRate, @ReadFromChannelsStaticCallback);
 if OldHandle <> 0
  then DiracDestroy(OldHandle);
end;
...
begin
 result := DiracGetProperty(Integer(dpPitchCorrectionBasicTuning), FDiracHandle);
end;
...
begin
 ...
 FIntDataPos := 0;
 FInternalData := Input;
 while SampleCount < SampleFrames
  do SampleCount := SampleCount + DiracProcess(Output, SampleFrames, Self, FDiracHandle);
end;

function TDiracLE.ReadFromChannelsCallback(Data: PDAVSingleFixedArray; const FrameCount: Integer): Integer;
begin
 ...
 try
  ...
  if GLibDiracHandle <> 0 then
   begin
    ...
    DiracGetInputBufferSizeInFrames := GetProcAddress(GLibDiracHandle, MakeIntResource(4));
