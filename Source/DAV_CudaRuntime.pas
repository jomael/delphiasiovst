...
type
  ...
  TCudaMallocArray = function (var aarray: Pointer; var desc: TCudaChannelFormatDesc;
    Width, Height : Cardinal): TCudaError; stdcall;
  ...
  TCudaMemcpy3DAsync = function(const p: PCudaMemcpy3DParms; Stream: TCudaStream): TCudaError; stdcall;
...
//  TCudaConfigureCall = function (dim3 gridDim, dim3 blockDim, Cardinal sharedMem __dv(0), Stream TCudaStream __dv(0)): TCudaError; stdcall;
  TCudaSetupArgument = function (const Arg: Pointer; Size: Cardinal; Offset: Cardinal): TCudaError; stdcall;
...
begin
 case CudaError of
  ceSuccess..ceNotReady : result := CCudaErrorStrings[Integer(CudaError)];
       ceStartupFailure : result := CCudaErrorStrings[35];
       ceApiFailureBase : result := CCudaErrorStrings[36];
 end;
end;
...
begin
 ...
 if CudaRuntimeDLL = 0
  then raise Exception.CreateFmt('File: %s could not be found', [CCudaRuntimeDLLName])
  else
   begin
